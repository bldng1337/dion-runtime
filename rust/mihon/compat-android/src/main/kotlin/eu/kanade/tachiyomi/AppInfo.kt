 ...
 * version metadata (name/code). Several extensions (e.g. cubari, bakkin,
 * foolslidecustomizable, magicaltranslators) read [versionName] at source
 * creation time — typically to build a user-agent or a `X-App-Version` header —
 * so the object must be present and resolvable on the classpath. Without it
 * these extensions fail to install with `NoClassDefFoundError: AppInfo`.
 *
 * Ported from the desktop `compat` layer (`eu/kanade/tachiyomi/AppInfo.kt`).
 */
object AppInfo {
    /**
