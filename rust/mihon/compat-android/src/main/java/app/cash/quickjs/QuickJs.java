...
public class QuickJs implements Closeable {
    ...
    /**
     * Compiles {@code sourceCode} into an opaque bytecode blob.
     *
     * <p>The real QuickJs serializes QuickJS bytecode so it can be cached and
     * later passed to {@link #execute(byte[])}. Rhino has no compatible
     * bytecode format, so this implementation returns an internal token that
     * stashes the source; {@link #execute(byte[])} replays it. This keeps the
     * compile-then-execute pattern used by some extensions (e.g. mangago)
     * working without a native QuickJS engine.
     *
     * @param sourceCode the JavaScript to compile
     * @param fileName   the source file name used in error reporting
     * @return an opaque bytecode token (internally a tagged byte array)
     * @throws QuickJsException if compilation fails
     */
    public byte[] compile(String sourceCode, String fileName) {
        ensureOpen();
        try {
            // Force a parse so syntax errors surface here (matching QuickJs).
            context.compileReader(
                scope,
                new java.io.StringReader(sourceCode),
                fileName,
                1,
                null
            );
            ...
            throw new QuickJsException(
                "Failed to compile JavaScript: " + t.getMessage(),
                t
            );
        }
        return CompiledScripts.INSTANCE.register(sourceCode, fileName);
    }

    /**
     * Executes a blob previously produced by {@link #compile(String, String)} and
     * returns the result coerced to {@code returnType}.
     *
     * @throws QuickJsException if the blob is unknown or evaluation fails
     */
    public Object execute(byte[] bytecode) {
        return execute(bytecode, Object.class);
    }

    /**
     * Executes a blob previously produced by {@link #compile(String, String)} and
     * coerces the result to {@code returnType}.
     *
     * @throws QuickJsException if the blob is unknown or evaluation fails
     */
    public <T> T execute(byte[] bytecode, Class<T> returnType) {
        ensureOpen();
        CompiledScripts.Entry source = CompiledScripts.INSTANCE.lookup(
            bytecode
        );
        if (source == null) {
            throw new QuickJsException("Unknown QuickJs bytecode blob");
        }
        return evaluate(source.first, source.second, returnType);
    }
