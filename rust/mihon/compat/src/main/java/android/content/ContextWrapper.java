...
/**
 * Minimal {@code ContextWrapper} stub for Mihon extensions.
 *
 * <p>In the real Android framework, {@code Application} (and many other
 * framework classes) extend {@code ContextWrapper}, which itself extends
 * {@code Context}. Extension bytecode routinely references methods through the
 * static type {@code android.content.ContextWrapper} (e.g. the Keiyoushi
 * helper calls {@code ContextWrapper.getSharedPreferences} on an
 * {@code Application} instance). The JVM must therefore be able to resolve and
 * link against this class, and {@code Application} must be assignable to it.
 *
 * <p>This stub provides the same default implementations that {@code Application}
 * previously carried (delegating to nothing useful / returning nulls), so that
 * the hierarchy matches Android: {@code ContextWrapper extends Context} and
 * {@code Application extends ContextWrapper}. Concrete behaviour is supplied by
 * the runtime's {@code FakeApplication} / {@code CustomContext} overrides.
 */
public class ContextWrapper extends Context {

    public ContextWrapper(Context base) {
        // Real Android attaches a base context here; we don't need delegation.
    }
