...
import J from "path";
var { argv: K } = globalThis.Bun;
import S from "handlebars";
function b(j, x) {
	return new Promise((C) => {
		process.stdout.write(`${j}${x ? ` (${x})` : ""}: `);
		let G = [];
		process.stdin.resume(),
			process.stdin.once("data", (L) => {
				G.push(L), process.stdin.pause();
				let z = Buffer.concat(G).toString().trim();
				C(z.length ? z : (x ?? ""));
			});
	});
}
...
async function R(j) {
	await P.mkdir(j, { recursive: !0 });
}
async function N(j, x) {
	await R(J.dirname(j)), await P.writeFile(j, x);
}
async function F(j, x) {
	return S.compile(j, { noEscape: !0 })(x);
}
function T(j) {
	let x = { yes: !1 },
		C = j[Symbol.iterator](),
	...
	while (!L.done) {
		let z = L.value;
		if (z === "-y" || z === "--yes") x.yes = !0;
		else if (z === "--description") {
			let B = C.next();
			if (!B.done) x.description = B.value;
		} else if (z.startsWith("--description="))
			x.description = z.split("=", 2)[1] ?? "";
		else if (z === "--url") {
			let B = C.next();
			if (!B.done) x.url = B.value;
		} else if (z.startsWith("--url=")) x.url = z.split("=", 2)[1] ?? "";
		else if (z === "--author") {
			let B = C.next();
			if (!B.done) x.author = B.value;
		} else if (z.startsWith("--author=")) x.author = z.split("=", 2)[1] ?? "";
		else if (z === "--icon") {
			let B = C.next();
			if (!B.done) x.icon = B.value;
		} else if (z.startsWith("--icon=")) x.icon = z.split("=", 2)[1] ?? "";
		else if (z === "--keywords") {
			let B = C.next();
			if (!B.done) x.keywords = B.value;
		} else if (z.startsWith("--keywords="))
			x.keywords = z.split("=", 2)[1] ?? "";
		else if (z === "--media") {
			let B = C.next();
			if (!B.done) x.media = B.value;
		} else if (z.startsWith("--media=")) x.media = z.split("=", 2)[1] ?? "";
		else if (z === "--name" || z === "-n") {
			let B = C.next();
			if (!B.done) x.name = B.value;
		} else if (z.startsWith("--name=")) x.name = z.split("=", 2)[1] ?? "";
		else if (!z.startsWith("-")) G.push(z);
		L = C.next();
	}
	if (G.length > 0) x.path = G[0];
	if (G.length > 1) x.name = G[1];
	return x;
}
async function M(j, x, C, G) {
...
async function U() {
	let j = T(K.slice(2)),
		x = await M(j.name, "Extension name", "my-extension", j.yes),
		C = q(x),
		G = x,
		L = await M(j.description, "Description", "A Dion extension", j.yes),
		z = await M(j.url, "Website URL", "https://example.com/", j.yes),
		B = await M(j.author, "Author", "Author", j.yes),
		V = await M(
			j.icon,
			"Icon URL",
		...
		W = y(
			await M(j.keywords, "Keywords (comma separated)", "example, dion", j.yes),
		),
		X = (
			await M(
				j.media,
			...
			.filter(Boolean),
		E;
	if (!j.path) E = J.resolve(".", C);
	else if (j.path.includes("/") || j.path.includes("\\")) E = J.resolve(j.path);
	else E = J.resolve(j.path, C);
	if (await P.exists(E))
		console.error(`Directory already exists: ${E}`), process.exit(1);
	let Y = await `{\r
  "name": "{{name}}",\r
  "version": "1.0.0",\r
  ...
  "type": "module",\r
	"scripts": {\r
		...
		"pushdev": "bunx dion-push-extension -d",\r
		"push": "bunx dion-push-extension",\r
		"format": "bunx biome format --write",\r
		"build": "bunx dion-bundle-extension",\r
		"check-types": "bunx tsc --noEmit"\r
	},\r
	"dependencies": {\r
		"@dion-js/runtime-lib": "workspace:*"\r
	},\r
	"devDependencies": {\r
		"@types/bun": "catalog:",\r
		...
		"@dion-js/config": "workspace:*",\r
		"dion-bundle-extension": "workspace:*",\r
		"dion-push-extension": "workspace:*",\r
		"dion-test-asserts": "workspace:*",\r
...
`,
		A =
			await `/** biome-ignore-all lint/style/noNonNullAssertion: These are tests so if they fail it is fine */\r
...
\r
beforeAll(async () => {\r
	client = new MockManagerClient("./.dist");\r
...
`,
		H = {
			...
			url: z,
			author: B,
			keywords: JSON.stringify(W),
			media_type: JSON.stringify(X),
			title: G,
		};
	await R(E),
		await N(J.join(E, "package.json"), await F(Y, H)),
		await N(J.join(E, "tsconfig.json"), Z),
		await N(J.join(E, "biome.json"), _),
		await N(J.join(E, "src", "main.ts"), $),
		await N(J.join(E, "src", "main.test.ts"), A),
		console.log(`
Created extension at ${E}`),
		console.log("Next steps:"),
		console.log(`- cd ${J.relative(process.cwd(), E)}`),
