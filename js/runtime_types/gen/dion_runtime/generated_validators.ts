...
export const AuthData = v.union([v.object({ Cookie: v.object({ loginpage: v.string(), logonpage: v.string() }) }), v.literal("ApiKey"), v.literal("UserPass")]);

export const Account = v.object({ domain: v.string(), user_name: v.nullable(v.string()), cover: v.nullable(v.string()), auth: AuthData, creds: v.nullable(v.record(v.string(), v.string())) });
...
export const SettingsUI = v.variant("type", [v.object({ type: v.literal("CheckBox") }), v.object({ type: v.literal("Slider"), min: v.number(), max: v.number(), step: v.number() }), v.object({ type: v.literal("Dropdown"), options: v.array(DropdownOption) })]);
