...
export const AuthCreds = v.variant("type", [v.object({ type: v.literal("Cookies"), cookies: v.record(v.string(), v.array(v.string())) }), v.object({ type: v.literal("ApiKey"), key: v.string() }), v.object({ type: v.literal("UserPass"), username: v.string(), password: v.string() }), v.object({ type: v.literal("OAuth"), access_token: v.string(), refresh_token: v.nullable(v.string()), expires_at: v.nullable(v.number()) })]);

export const AuthData = v.variant("type", [v.object({ type: v.literal("Cookie"), loginpage: v.string(), logonpage: v.string() }), v.object({ type: v.literal("ApiKey") }), v.object({ type: v.literal("UserPass") }), v.object({ type: v.literal("OAuth"), authorization_url: v.string(), token_url: v.nullable(v.string()), client_id: v.string(), client_secret: v.string(), scope: v.nullable(v.string()) })]);
...
export const SettingsUI = v.variant("type", [v.object({ type: v.literal("CheckBox") }), v.object({ type: v.literal("Slider"), min: v.number(), max: v.number(), step: v.number() }), v.object({ type: v.literal("Dropdown"), options: v.array(DropdownOption) }), v.object({ type: v.literal("MultiDropdown"), options: v.array(DropdownOption) }), v.object({ type: v.literal("CustomUI"), ui: v.lazy(() => CustomUI) })]);
...
export const MixedContent: v.GenericSchema = v.variant("type", [v.object({ type: v.literal("Text"), content: v.string() }), v.object({ type: v.literal("CustomUI"), ui: v.lazy(() => CustomUI) }), v.object({ type: v.literal("Table"), columns: v.array(v.lazy(() => Row)) })]);
